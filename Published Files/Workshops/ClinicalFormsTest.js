...
//Create Date and Timestamp and day for file names 
function myDayandDateString(){
...
// The function gets the full name of agent based on computer login information
var getUserFullName = app.trustedFunction(
function () {
...
var BCLSend = app.trustedFunction(function(){
    ...
    if(this.getField("BCLv1") === null){
        var BCLOldAlert = app.alert ("You are using the incorrect or outdated BCL form for this button \n\n" + "The current BCL Form will open from the Blank Forms Button",0,0);
...
var BCLXFin = app.trustedFunction(function(){
    // Checks if current open document if valid by checking hidden field on form
    if(this.getField("BCLv1") === null){
        var BCLAlert = app.alert ("You are using the incorrect or outdated BCL form for this button \n\n" + "The current BCL Form will open from the Blank Forms Button",0,0);

        if (BCLAlert == 1){
            app.openDoc("/uscplatxdfs001p/CLIENT/OPERATIONS/Customer Service/Resources/zFiles/Billing Correction (BCL).pdf");
        };    
    } else {
        // Sets up app alert to ask if it is a No Charge
...
var CRTSend = app.trustedFunction(function(){
    // Checks if current open document if valid by checking hidden field on form
    if (this.getField("CRTv1") === null){

         var CRTAlert = app.alert ("You are using the incorrect or outdated Clinical Resolution Ticket form for this button \n\n" + "The current CRT Form will open from the Blank Forms Button",0,0);
...
var CourierSend = app.trustedFunction(function(){
    ...
    if (this.getField("DATERow1") === null){
        ...
        if (CourierAlert == 4){
            app.beginPriv();
            this.saveAs("/uscplatxdfs001p/CLIENT/SHARED/Austin Daily Courier After Hours Pickup Log/" + myDayandDateString() +" Courier Log " + getUserFullName() +" .pdf");
            app.endPriv();
            this.closeDoc(true);
        }
    }
...
app.addMenuItem({cName:"In Transit STAT", cParent:"Submit Clinical Forms", cExec:"InTransitSTATSend();"});
app.addMenuItem({cName:"Courier Log", cParent:"Submit Clinical Forms", cExec:"CourierSend();"});

var ClinicalFormsMenu = app.trustedFunction(function(){
    var cRtn = app.popUpMenu("BCL","CRT","SRF","CAR",["Repeats","Start Repeat","Finish Repeat"],"Maternal Recal, In Transit STAT","Courier Log");
    if(cRtn){
